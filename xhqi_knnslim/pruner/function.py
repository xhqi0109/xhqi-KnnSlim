...
class BasePruningFunc(ABC):
    ...
    def xhqi_get_weight_masked(self, weight, keep_idxs, pruning_dim):
        device = weight.device
        weight = weight.detach()
        
        # 创建一个与 weight 形状相同的 mask，所有元素初始化为 False
        mask = torch.zeros_like(weight, dtype=bool, device=device if device else weight.device)
        
        # 在指定维度上将 mask 中对应位置设置为 True
        mask[(slice(None),) * pruning_dim + (keep_idxs,)] = True
...
class ConvPruner(BasePruningFunc):
    ...
    def xhqi_weight_masked_out_channels(self, layer: nn.Module, weight_pt:torch.tensor, param_name:str, idxs: Sequence[int]) -> torch.tensor:

        keep_idxs = list(set(range(layer.out_channels)) - set(idxs))
        keep_idxs.sort()
        if param_name == "weight":
            if not layer.transposed:
                weight_masked_pt = self.xhqi_get_weight_masked(weight_pt, keep_idxs, 0)
            else:
                weight_masked_pt = self.xhqi_get_weight_masked(weight_pt, keep_idxs, 1)
        elif param_name == "bias" and layer.bias is not None:
            weight_masked_pt = self.xhqi_get_weight_masked(weight_pt, keep_idxs, 0)
        else:
            raise ValueError("Unknown param_name of : {}".format(param_name))
        return weight_masked_pt

    def xhqi_weight_masked_in_channels(self, layer: nn.Module, weight_pt:torch.tensor, param_name:str, idxs: Sequence[int]) -> torch.tensor:
        keep_idxs = list(set(range(layer.in_channels)) - set(idxs))
        keep_idxs.sort()
        layer.in_channels = layer.in_channels - len(idxs)
        if layer.groups>1:
...
class DepthwiseConvPruner(ConvPruner):
    ...
    xhqi_weight_masked_in_channels = xhqi_weight_masked_out_channels
...
class ParameterPruner(BasePruningFunc):
    ...
    def xhqi_weight_masked_out_channels(self, tensor, weight_pt:torch.tensor, param_name:str, idxs: Sequence[int]) -> torch.tensor:
        keep_idxs = list(set(range(tensor.data.shape[self.pruning_dim])) - set(idxs))
        keep_idxs.sort()
        weight_masked_pt = self.xhqi_get_weight_masked(weight_pt, keep_idxs, self.pruning_dim)
        return weight_masked_pt
